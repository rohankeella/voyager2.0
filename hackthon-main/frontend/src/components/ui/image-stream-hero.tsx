...
/* eslint-disable @next/next/no-img-element -- Decorative repeated image corridor uses native lazy images. */
...
export type ImageStreamHeroProps = {
  /**
   * Images on the left rail, also used on the right unless rightImages is supplied.
   * Fewer than `cards` simply repeat.
   */
  images: StreamImage[];
  /** Separate image sequence for the right rail. */
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {[{ name: right, images: rightImages }, { name: left, images }].map(({ name, images: railImages }) =>
            Array.from({ length: cards }, (_, i) => {
              const img = railImages[i % Math.max(railImages.length, 1)];
              return (
                <div
                  key={`${name}-${i}`}
